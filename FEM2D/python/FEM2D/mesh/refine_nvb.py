...
def refine_nvb_cpp(mesh, marked, debug=False, timer=None):
    ...
    with _timed(timer, "closure"):
        ...
        cell_ref_faces = cpp_backend.cell_ref_faces_from_refedges(
            mesh.topology.faces,
            refedges,
        )
        r = cpp_backend.close_marked_faces_nvb(
    ...
    with _timed(timer, "refine_cells_loop"):
        ...
        with _timed(timer, "refine_cells_cpp_call"):
            r = cpp_backend.refine_cells_nvb(
    ...
    with _timed(timer, "bdry_labels"):
        #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        mesh2.labels.boundary = cpp_backend.boundary_edges_to_faces_all(
...
# ====================================================================== #
def refine_nvb_python(mesh, marked, debug=False, timer=None):
    """
    Variant of refine_nvb with preallocated global cell/refedge/label arrays.
    Intended for timing comparison against refine_nvb.
    """
    ...
    with _timed(timer, "refine_cells_loop"):
        ...
        for icell in range(ncells):
            ...
            if nnew > max_new_cells:
                raise RuntimeError(
                    f"max_new_cells exceeded: {nnew=} > {max_new_cells=}"
                )
