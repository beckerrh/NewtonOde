...
class P1general():
    ...
    def setMesh(self, mesh, innersides=False):
        ...
        if innersides: self.mesh.construct_inner_faces()
    ...
    def computeFormTransportCellWise(self, du, u, data, type):
        ...
        ndofs, dim, dV, dofspercell = self.nunknowns(), self.mesh.dimension, self.mesh.geometry.cell_volumes, self.dofspercell()
    ...
    def computeMatrixTransportCellWise(self, data, type):
        beta, betart = data.betacell, data.betart
        ndofs, dim, dV, dofspercell = self.nunknowns(), self.mesh.dimension, self.mesh.geometry.cell_volumes, self.dofspercell()
        # nfaces, dim, dV = self.mesh.nfaces, self.mesh.dimension, self.mesh.geometry.cell_volumes
        cellgrads = self.cellgrads[:,:,:dim]
        if type=='centered':
