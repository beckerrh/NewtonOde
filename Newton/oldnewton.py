...
class Newton:
    ...
    def __init__(self, **kwargs):
        ...
        self.printer = printer.Logger(verbose=self.verbose, types=types, name=self.name)
        ...
        if self.verbose_bt:
            ...
            self.printer_bt = printer.Logger(types, name='bt')
    ...
    def solve(self, x, **kwargs):
        ...
        for iteration in range(maxiter):
            ...
            if res is None:
                result_merit = self.nd.computeResidual(x)
                res = result_merit.residual
                meritvalue = result_merit.meritvalue
                xnorm = result_merit.norm_X
            ...
            if hasattr(self.nd,'update_rule'):
                ...
                if gradient_iteration:
                    btresult_old = btresult if 'btresult' in locals() else None
                    if hasattr(self.nd, 'call_back_backtrack_failed'):
                        kwargs = {
                            'btresult': btresult_old,
                            'dx': dx,
                            'r': res,
                            'dxnorm': dxnorm,
                            'dxnorm_old': dxnorm_old
                        }
